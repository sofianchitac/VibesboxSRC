...
each source into the sum bus over PipeWire. Per-source mute is implemented as *unlink* —
...
**`ndi_transmitter.py`** reads the sum bus straight out of PipeWire and transmits it as an
NDI stream via the NDI SDK (loaded through `ctypes`). It is deliberately dumb and always
running — it does no format decisions and has no modes, so there is nothing in it to get out
